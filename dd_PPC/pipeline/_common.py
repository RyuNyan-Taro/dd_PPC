...
def _preprocess_data(datas: pd.DataFrame, sc: StandardScaler | None = None) -> tuple[pd.DataFrame, StandardScaler, list[str]]:
    ...
    return pd.concat([_datas_std.reset_index(drop=True), _datas_category.reset_index(drop=True), _datas_aggregates.reset_index(drop=True)], axis=1), sc, list(category_cols)
